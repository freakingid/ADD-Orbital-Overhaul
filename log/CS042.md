# CS042 — Twelve event sounds, a ceremony cross-fade, one cargo mass, and the Scoop redesigned

Full P0–P10, GATE A, GATE C and P11 build log, moved out of `STATUS.md` at the CS042 closing phase
per the standing per-changeset log convention (`CLAUDE.md`). Baseline `71ab1bc` ("off-cycle: GDD
accuracy pass"), `GAME_VERSION` **1.0.0.40 → 1.0.0.42** (see P11 — the phase doc specified `.41`; Paul redirected at the close so the last field tracks the changeset number). Debug registry **110 → 117**, `LEVERS`
**18 — unmoved**. `orbital-overhaul.html` 1,040,713 → **1,081,177** bytes. Suite closes at **180
files, 180 passed, 0 failed, 0 skipped, 0 timed out**, up from 171 files at the baseline — nine new
test files, one per build phase plus this one. Companion docs `PLANNED-FEATURES-CS042.md` and
`IMPLEMENTATION-PHASES-CS042.md` move to `archive/` at this same closing phase. `CS042-GATE-A.md`
does **not** move — it stays at the repo root as the permanent verbatim record of GATE A's two
copy-out blocks, on `CS039-VOICE-WORKLIST.md`'s precedent, because three phases were pinned
byte-for-byte against it and those pins must keep resolving.

---

## §0 — What shipped, in one page

**Six mechanics and one instrument set.** Twelve `AudioSys` event SFX so every announced event
sounds with or without Dan's voice, composed in a new lab and ported verbatim (P1/P3/P4). A
cross-fade over both level-end and game-over ceremonies, taken unedited from a second new lab's
preset (P2/P5). Health supply levelling — five changes behind three knobs, one of them reversing
CS040 P1 (P6). **One mass, one force**: four hand-tuned cargo divisors retired for a single
`shipMass()`, plus the Engine's fuel burning only under load (P0/P7). The Scoop redesigned on both
sides — levels 6–7 with flanking capture orbs and a raised level-1 floor (P8), then a loss rate
reversed 5 → 2 with a health-reserve spare (P9). Menu navigation key repeat, up/down only (P10).
And GATE C's one actionable finding, a render-only answer to the Scoop reading as hull (P11).

**Three forks and thirteen flags, all closed.** FORK-CS042-A (the Scoop cap vs the mouth's span),
-B (SFX at the trigger site, never inside `_emit()`), -C (the Engine gets no cargo-independent arm —
withdrawn at Paul's direction), -D (the Engine is bounded by laden thrust-seconds), and FORK-S1
carried in from CS040 (payload loss must not cost a scoop level — the build was already right).
FLAG-CS042-a through -m: -f and -j closed as "does not ship" at GATE A, -k superseded outright by
§6.8, and -l/-m closed at GATE C as "fine as shipped". ⛔ **FLAG-CS041-b — `CLAUDE.md`'s own 50 KB
ceiling — is the one flag this changeset OPENED rather than closed**; see P11 below.

**The one null result, and it was load-bearing.** GATE A's handling third came back empty after
three passes through `tools/handling-lab.html`, with Paul's own note that he was *"not feeling super
confident about how this testing went."* That was read as a verdict on the instrument rather than on
the question — a mock ship on an empty field has no enemies, no dock, no chain to lose and nothing
to be late for — and it is what produced §6.8's "one mass, one force" from the other end. P7 was the
only phase ever blocked in this changeset, and that is what unblocked it.
...
**The tuning pass was ONE change and it was render-only.** GATE C answered nine of its ten questions
"ship as-is", so nothing numeric moved: no lever, no knob, no constant retune, and both open flags
closed at their shipped values. G4 was the exception — the level-6/7 orbs and the scoop mouth read
as *ship geometry*, solid glow-strokes implying hittable hull, when nothing the Scoop draws has ever
been collidable. Paul named two candidate directions and chose neither, delegating the pick to this
phase.

⛔ **The pick is a dashed field stroke, and transparency was rejected for a structural reason rather
than a stylistic one.** `ctx.globalAlpha` inside `Ship.draw()`'s scoop block belongs to the
level-end grace pulse's SET-DRAW-RESTORE — which is exactly why CS042 P8 one phase earlier made the
level-7 tether a *colour* choice and wrote "faint is a colour choice, not an alpha one" beside it.
Composing a second alpha writer there is possible but reverses that call; more decisively, **a faint
outline is still an outline.** It dims a boundary without stopping it reading as one, and G4 is
about what the shape means, not how loud it is. A broken outline makes a different claim: it does
not read as a surface at all. `glowStroke`'s `shadowBlur` paints a continuous halo through the gaps
while the bright core stays broken — an energy field rather than a dashed line on a blueprint.

**Implementation, and the one trap in it.** Two plain constants (`SCOOP_FIELD_DASH` /
`SCOOP_FIELD_GAP`, both 6 — pure presentation, on the `CAPTION_LINGER` precedent, deliberately **not**
registry rows, so the registry stays at 117) and one `ctx.setLineDash()` window wrapping the mouth V,
both orbs and both level-7 tethers. ⛔ **The clear is mandatory, not tidiness:** `drawPoly()` wraps
its own `save()`/`restore()` pair, which *preserves* a dash rather than dropping it, so an unclosed
window would dash the hull, the thruster flame, the shield and every entity drawn after the ship.
That is the test's load-bearing mutation check — the counterfactual build (arm kept, clear removed)
is constructed and driven, and the hull is shown to stroke inside the armed window. `drawPoly` and
`glowStroke` stay dash-agnostic; this is the one `setLineDash` site in the build, asserted as such.
`tools/scoop-lab.html` was resynced in the same edit so its preview still matches what the game
draws (GDD §2.14.1 states that contract).

**Cost, since Paul flagged it as a real evaluation rather than a knob turn:** two `setLineDash()`
calls per drawn frame, on the ship alone, and only while a scoop exists.

⚠ **It ships unseen in a browser.** 6/6 px was sized off the shapes it strokes (13.5 dashes around an
L6 orb ring, 17.6 around an L7) rather than picked round, but nobody has looked at it. A retune edits
the two constants directly.

**Test.** `scratchpad/test-cs042-p11.js` — 147 assertions in seven sections, zero skips. §B drives
the real `Ship.draw()` through the harness's recording ctx at every level 0–7 and checks the arm/clear
indices against the mouth, orb, tether and hull stroke indices. §C retunes both constants in a
substituted build and confirms the armed pattern moves with them, so the call site is proven to read
the names rather than two coincidental literals. §E is the render-only pin measured against the
phase's own literal parent SHA: `inScoopBox()`, `buildScoopSteps()` and `damageShip()` are
byte-identical, all four scoop tables are unmoved, six knobs GATE C closed are unmoved, and **12,600
capture probes across eight levels × five headings match the parent exactly**. §G asserts nothing
between the arm and the clear writes `ctx.globalAlpha`, so the grace pulse still owns that channel.
⛔ **No file-scope pin**, per the standing rule that a closing phase rewrites documents by
instruction and so cannot survive one.

**Docs.** GDD §2 updated by named subsection — §2.1 and §2.10/§2.10.1/§2.10.2 plus §1.1 and §3's
Chain physics row for the one-mass model; §2.7, §2.12 and §2.14 for health supply, the reserve's
second job and the Engine's burn condition; §2.14.1 rewritten for the cap-vs-span split, the orbs,
the level-1 floor, the reversed loss rate and P11's own field stroke; §2.8 for the twelve sounds and
the trigger-site rule; §2.16 for menu repeat; §2.19 for the registry's 110 → 117 growth; §2.20.1 for
GATE C's G10. Seven §0 index rows gained the things they were missing (P8 flagged §2.14.1's as a
defect in §0; it is repaired). `DIFFICULTY-LEVERS.md` §4 gained six rows for the seven new knobs and
§6 a GATE C retune entry; `TELEMETRY-ANALYSIS-GUIDE.md`'s constants table was corrected on four rows
and gained five. **All ~35 GDD §0 size rows were re-measured** with `scratchpad/gdd-sizes.py --check`
— sixteen had drifted — and the preamble's own two aggregate figures with them, one of which
(§4–§7's "~14 KB", actually 21.8 KB) had been stale independently of this changeset.

⛔ **The GDD's build stamp is current for the first time in sixteen changesets, and the checklist now
owns it.** Line 3 read "CS041 P9 … 1.0.0.40 … registry 110" and had no owner; `CLAUDE.md`'s
closing-phase rule now re-reads it in the same step that re-measures §0's sizes. That is the
structural fix STATUS.md had recorded as "the cheap way to stop this recurring, and is not yet done."

### ⛔ `CLAUDE.md` CROSSED ITS CEILING, THE VALVE FIRED, AND THE VALVE IS NOW EXHAUSTED

The phase prompt predicted the crossing and prescribed the remedy, and the remedy was applied in
full: **`### Audio` went 5,252 → 4,026 bytes (5.13 → 3.93 KiB)**, its reasoning relocated undeleted
to `RATIONALE.md` under `#music`, `#voice`, `#voice-queue`, `#captions` and two new anchors,
`#voice-repeat` (CS038 P4's third mechanism) and `#event-sfx` (CS042's trigger-site rule). Every
rule stayed and now names its anchor.

⛔ **That was the whole ~1.2 KB the valve had to give.** After firing, **no section in the file is
over ~4 KB** — the largest is `### Audio` itself at 3.93 KiB, then `### Save data` at 3.71 — and the
valve's own ⚠ SETTLED clause says a section under ~4 KB is not a candidate however long the file
gets. So the mechanism has no next candidate, at its current threshold, ever.

CS042's own rules then took the file past the line anyway, **and their reasoning was relocated
before it was ever written into `CLAUDE.md`** — three further new anchors, `#cs042-health`,
`#cs042-mass` and `#cs042-scoop`, carry it, and `CLAUDE.md` holds bare imperatives pointing at them.
`RATIONALE.md` grew 43,246 → 62,027 bytes across the eight anchors, which is the valve working as
designed: bytes moved from the file every session pays for to one on an on-demand contract.

**Final measurement: 52,480 bytes / 880 lines = 51.25 KiB, ~1.25 KB over the 50 KB ceiling.**
(HEAD was 50,657 / 857 = 49.47 KiB.) ⛔ **This phase deliberately did not close the gap**, because
the only three ways to are all Paul's calls, not a phase's: lower the ~4 KB section threshold so the
valve has candidates again, raise the ceiling (which `CLAUDE.md` itself has always called "a first
guess"), or accept the overrun. The two things a phase must *not* do are silently drop a rule the
changeset needs in order to fit, and silently valve an under-size section against the rule's own
explicit prohibition. **FLAG-CS041-b is therefore live and blocking nothing, but it is the first
open question of CS043.**

### Suite

**180 files, 180 passed, 0 failed, 0 skipped, 0 timed out** (exit 0), `node --check` clean on the
extracted script, no `test-f6.js` flake rerun needed. Zero skips is asserted, not assumed — the run
reports the count directly and `SKIP_TAG` appears nowhere in the output. Nine files changed beyond
the new one: `test-cs016-p5.js` and `test-cs021-p4.js` are the two live version pins, re-pointed
1.0.0.40 → 1.0.0.42 as the standing rule requires; every phase-local mirror-image pin
(`!== "1.0.0.37"`, `!== "1.0.0.19"`) is permanently true and was correctly left alone.

### ⛔ The version is 1.0.0.42. The phase doc said `.41`, this phase shipped `.41`, and both were wrong

`IMPLEMENTATION-PHASES-CS042.md` P11 item 7, `PLANNED-FEATURES-CS042.md` §0 and that doc's own
**Base:** line all say `1.0.0.40 → 1.0.0.41`, and the closing phase shipped exactly that. Paul
caught it: *"Because this was changeset 42, the version should be 1.0.0.42."*

⛔ **This was a correction to a scheme that already existed, not a new rule** — which is the part
worth recording, because it means the failure was a reading failure. `GAME_VERSION`'s own comment
block has said since CS010 P0: *"Changeset = the CS### number, monotonically increasing, never
resets — the 4th segment IS the changeset number"*, with thirteen `"Still tracking the changeset
number"` lines under it. The phase doc's *"bump 1.0.0.40 → 1.0.0.41"* was written as the next
integer, that stopped being the changeset number one changeset earlier, and the closing phase took
the phase doc at face value without reading the constant it was editing.

**How they fell out of step:** CS041 shipped no build byte and resolved FORK-CS041-A to "no version
bump" — correctly, since there was nothing to stamp — so the last-shipped version was `.40` while
the next changeset was 42. ⛔ **`.41` is now a deliberate gap and must not be back-filled**, the
identical rule and almost the identical reason as `.23`'s, which that same comment block has carried
since CS024 P7. Both skips are recorded there, at the constant, where the next closing phase will
be looking.

**Nothing in the suite caught it, and nothing could have.** The two live pins assert HEAD's literal,
so they check that a bump *happened*, not that it was *right* — they passed on `.41` and pass on
`.42`. Asserting the changeset number would need it machine-readable somewhere, which it is not.

The re-bump is the same edit as any other: the constant, the **two live pins**
(`test-cs016-p5.js` §A, `test-cs021-p4.js` §A), the GDD's build stamp, `STATUS.md`'s header, and
`TODO.md`'s snapshot line. The two phase-local mirror-image pins are unaffected for the reason
they exist — `!== "1.0.0.37"` and `!== "1.0.0.19"` are permanently true whatever HEAD reads. ⛔
**The archived planning docs were NOT rewritten**: `archive/` is a historical record, they said
`.41` when they were written, and this log is where the supersession belongs. The standing rule
going forward is in `DECISIONS.md` and in `STATUS.md`'s Next up — a changeset that ships no build
byte still bumps nothing and still leaves a gap; it does not hand its number to the next one.

---

## Headline number

**CS042's headline is the four cargo divisors becoming one mass, because it is the only change here
that made the whole model tunable rather than making one number better.**

`CARGO_THRUST` (0.07), `CARGO_MAXSPD` (0.035), `CARGO_MASS` (0.10) and `CARGO_TURN` (0.0) were four
separately hand-tuned coefficients for four symptoms of one physical fact, and three full GATE A
passes through a purpose-built lab produced no set Paul trusted. `M = 1 + chainMass() × 0.07`
replaced all four, with acceleration **byte-identical at every chain length** to every build since
CS010 — so the thing that changed is not the number, it is that there is now one number. Coast went
5.3 s → 14.3 s, turn 241 → 90 °/s, and the momentum tug lost the arbitrary 1.4 clamp that had
flat-spotted it from 14 nodes to 24.

Two consequences were flagged before shipping and both closed at GATE C as right: **top speed is no
longer penalised at all** (a full haul can reach 520 where it used to clamp at 283 — mass limits
agility, not speed) and **rotation now is** (reversing a flag Paul had closed the other way one gate
earlier). The measured cost is honest and is recorded rather than softened: only the tug's endpoints
were held, and its mid range is weaker, worst at m = 14 (−21%).

**Second headline, on the docs:** `CLAUDE.md`'s size valve was invented at CS041 P1 and fired for the
first time here — and firing it once emptied it. See P11 above.

---

## GDD version history

CS042 (P0–P10, GATE A, GATE C, P11) — Twelve event sounds, a ceremony cross-fade, one cargo mass, and
the Scoop redesigned. `GAME_VERSION` **1.0.0.40 → 1.0.0.42** — the last field tracks the changeset number from here on (Paul, at this close), and `.41` is permanently unused because CS041 shipped no build byte; debug registry **110 → 117**; `LEVERS`
**18, unmoved**. Added twelve `AudioSys` event SFX (`cargofull`/`cargolost`/`chainsever`/`guardblock`/
`levelup`/`hullfull`/`hullrelief`/`hullcritical`/`powertag`/`powerfade`/`haulsize`/`megadelivery`),
composed as three candidates apiece in a new `tools/sfx-lab.html`, picked by ear at GATE A and ported
verbatim, each fired at its **trigger site immediately above the `VoiceSys.say()`** so an announced
event still sounds when the voice channel drops, parks, suppresses or is switched off (FORK-CS042-B);
`guardblock()` **replaces** the `shieldPing()` the chain guard used to borrow. Cross-faded both
ceremonies from a second new instrument, `tools/ceremony-lab.html`, taking its "Cross-fade" preset
unedited — fourteen transitions, **no timing moved**, each dismissal keeping a render-only ghost
(`levelDoneOut`, `celebrationOut`) because the fields the rest of the build gates on must still go
null; reverses CS034 P7 by gating the GAME OVER stack on `!game.celebration` again. Levelled the
health supply (§2.7/§2.14): the one-at-a-time gate **moved into `spawnHealthPowerup()`**, reversing
CS040 P1 so no route can put a second Health on the field; a global spawn lock bounds the rate; the
milestone interval grows with the level and a crossing pays only below 70% hull; the four ambient
pity-cadence pairs widened to 30/45/10/16 s. Replaced four cargo handling divisors with **one mass**
(§2.1/§2.10/§3.4): `shipMass() = 1 + chainMass() × CARGO_UNIT_MASS`, dividing acceleration, the drag
**rate**, the turn rate and the tug's `(M−1)/M`; `CARGO_THRUST`/`CARGO_MAXSPD`/`CARGO_MASS`/
`CARGO_TURN` deleted, `CHAIN_TUG` rescaled 26 → 58 to hold a full chain's yank fixed, acceleration
byte-identical at every chain length, top speed now a flat rail (FLAG-CS042-l) and rotation penalised
(FLAG-CS042-m) — both closed at GATE C as shipped; §3.4's stability envelope re-validated at 4.112 px,
unmoved. The Engine's tank now burns only under load. Redesigned the Scoop (§2.14.1): cap 5 → **7**
with `SCOOP_MOUTH_LEVELS` split out so the mouth curve keeps its own five-step span (FORK-CS042-A),
levels 6–7 granting **flanking capture orbs** — the Scoop's first lateral reach — the level-1 floor
raised from 713 to 2,200 px² of capture box, the loss rate reversed 5 → **2** hits per level
(undoing v3.4 P3's stated intent), and a banked Health charge given a second job: above 70% hull it
**spares a scoop level** instead of healing. Added menu navigation key repeat for both devices,
up/down only, keeping the browser's own `e.repeat` guard. GATE C's one actionable finding was
render-only and closes the changeset: every scoop shape now strokes **dashed**, so the mouth and orbs
read as a projected field rather than as hittable hull. Suite 171 → **180 files, 180 passed, 0
failed, 0 skipped**. `CLAUDE.md`'s CS041 size valve fired for the first time (`### Audio` 5.13 → 3.93
KiB, into six `RATIONALE.md` anchors) and is now exhausted — the file closes at 51.25 KiB, over its
50 KB ceiling, with FLAG-CS041-b open for Paul.
