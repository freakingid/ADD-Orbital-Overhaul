...
//
// Checks:
//  (A) GAME_VERSION === "1.0.0.34" (unprefixed).
//  (B) a fresh run's record carries build === "1.0.0.34". ⚠ CS034 P7 moved WHERE that stamp is
...
(function sectionA() {
  console.log("(A) GAME_VERSION === \"1.0.0.34\"");
  assert(GAME_VERSION === "1.0.0.34", "A: GAME_VERSION is exactly \"1.0.0.34\" (unprefixed)");
})();

// ================= (B) a fresh HighScores.add() stamps the new build ==================
(function sectionB() {
  console.log("(B) a fresh run's record carries build === \"1.0.0.34\"");
  HighScores.entries = [];
  const rec = HighScores.add(makeRunResult());
  assert(rec.build === "1.0.0.34", "B: new record's build field is \"1.0.0.34\"");
  ...
  assert(makeRunResult().build === "1.0.0.34", "B: makeRunResult() is what stamps it");
})();
