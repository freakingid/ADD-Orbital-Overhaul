...
//
// Checks:
//  (A) GAME_VERSION === "1.0.0.18" (unprefixed).
//  (B) a fresh HighScores.add() stamps build === "1.0.0.18".
...
(function sectionA() {
  console.log("(A) GAME_VERSION === \"1.0.0.18\"");
  assert(GAME_VERSION === "1.0.0.18", "A: GAME_VERSION is exactly \"1.0.0.18\" (unprefixed)");
})();

// ================= (B) a fresh HighScores.add() stamps the new build ==================
(function sectionB() {
  console.log("(B) fresh HighScores.add() stamps build === \"1.0.0.18\"");
  HighScores.entries = [];
  const rec = HighScores.add({ initials: "AAA", score: 100, wave: 1, delivered: 1 });
  assert(rec.build === "1.0.0.18", "B: new record's build field is \"1.0.0.18\"");
})();
