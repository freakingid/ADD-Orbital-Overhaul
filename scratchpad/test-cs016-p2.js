...
(function sectionA_shapes() {
  ...
  // REPOINTED BY CS038 P1 ("Credits" before "Back", 4 -> 5) and CS040 P6 ("Telemetry" before "Back",
  // 5 -> 6). This phase's own claim — that Achievements and High Scores are GONE from Options — is
  // indifferent to either insert and is asserted on its own two lines below rather than riding on the
  // row count.
  assert(eqJSON(A.MENU_OPTIONS, ["Sound / Music", "Controls", "Difficulty", "Credits", "Telemetry", "Back"]),
    `A: MENU_OPTIONS shrank 6 -> 4, then CS038 P1 added Credits, then CS040 P6 added Telemetry; got ${JSON.stringify(A.MENU_OPTIONS)}`);
...
(function sectionD() {
  ...
  // ⚠ CS034 P6: what confirm does on the viewer NOW — it stays on the screen and raises the reset
  // modal. Asserted here so the loop above cannot pass merely because the action was dropped from it.
  A.quitToTitle();
  g.menu.index = A.MENU_TITLE.indexOf("Achievements");
  A.menuInput("confirm");
  assert(g.menu.screen === "achievements", "D: (setup) on the viewer");
  A.menuInput("confirm");
  assert(g.menu.screen === "achievements" && !!g.menu.modal,
    "D: ⚠ CS034 P6 — confirm on the viewer STAYS and opens the reset modal (it used to exit)");
  A.menuInput("back");   // dismiss the dialog; `back` on a modal is a cancel, not a screen change
