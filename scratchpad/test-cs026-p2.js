...
(function sectionH() {
  ...
  eq(X.DEBUG_ENTRIES.length, 81, "H: the registry holds 81 value entries (75 + junkSplit's three + CS026 P3's earlyWorldLevels) [CS026 P4 -> 81]");
  eq(X.DEBUG_VARS.filter(v => !v.header).length, 81, "H: ...and DEBUG_VARS agrees");
  eq(Object.keys(X.DEBUG).length, 81, "H: ...and the native DEBUG map");
  eq(Object.keys(X.debugShown).length, 81, "H: ...and the display map");
