...
(function sectionB() {
  ...
  // ⛔ REPOINTED AGAIN BY CS043 P1, back to the shape it had before CS036 P2: the clear no longer holds
  // behind "Level N Complete" and there is no confirm to drive, because the beat is DELETED — the
  // wave-clear latch calls nextWave() inline on the frame the field empties. The CLAIM is unchanged and
  // is the whole point of the section: what the clear carries over, and what it adds on top.
  let guard = 0;
  while (game.wave === waveBefore && guard++ < 200) update(0.1);

  assert(guard < 200, "B: nextWave fired within the guard window (a debris-empty clear, inline)");
