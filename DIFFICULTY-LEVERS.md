...
| `deliveryFloatRise` / `deliveryFloatSize` / `deliveryFloatSizeStep` / `deliveryFloatSizeMax` / `deliveryFloatHold` / `deliveryFloatFade` | Flat knobs (150 px/s / 16 px / 1.0 px per piece / 48 px / 0.00 s / 1.20 s), CS026 P4 shape + P6 tuning, CS034 P8 GATE A retune + size/hold/fade split, CS035 P1 second-lab-session retune, **DELIVERY** section | **A floater's speed, size and fade are look-calls on a REWARD, not pressure axes.** They tune how the per-Debris `"+pts"` column at the dock reads; nothing about how hard the level plays. Scaling them by level would be backwards twice over — the delivery payoff is the player's own reward, and a deeper level pays *more*, so a level-scaled fade would make the biggest numbers the hardest to read. All flat by construction — **no floor/ceil/steps triple, no `▼`/`↳`, no `carriesTo`, no `LEVERS` entry** — and all knob-only, with no shipped constant behind them (the `magnetResumeDelay` idiom, not the `debrisBounceRestitution` one). ⛔ `deliveryFloatLife` (CS026 P4's single life knob) is **retired by CS034 P8** — its two readers (the delivery ticker's construction and the incidental floater's push) both moved to the `deliveryFloatHold`/`deliveryFloatFade` pair, which `FloatText`'s new optional `fade` field reads directly. `DELIVERY_FLOAT_DY` (22 px, the floaters' offset above the ship) is a **frozen constant and not even a knob**: it is a fixed nudge clear of the hull. `DELIVERY_FLOAT_ANCHOR_FRAC` (CS035 P1: back to 0.50, after CS034 P8's GATE A raised it to 0.75) is likewise a **fixed placement constant, not a knob** — see CLAUDE.md's Build rules. ⛔ `DOCK_OFFLOAD_INTERVAL` (0.05 s) is likewise NOT a knob and was deliberately left alone at the CS026 gate — see §6. |
...
| `healthSpawnLock` | Flat knob (12 s, `def` from `HEALTH_SPAWN_LOCK`), CS042 P6, **POWERUPS** section | **A supply RATE bound, and FLAG-CS042-c closed it flat on purpose.** Armed by every Health spawn route and respected by all of them, it says how often the field may gain a Health pickup at all; at 0 it is disabled outright, the clean A/B for that half. Making it scale with the level was considered and declined: `game.wave` is the game's one difficulty clock (§2) and a second scaling rule needs its own reason, which "how often health may appear" does not have — the *pity* response already rides hull, a per-run player state. Note its partner bound, the one-at-a-time gate inside `spawnHealthPowerup()`, has **no knob at all**: whether two may coexist is a yes/no design rule, not a dial. |
| `repairMilestoneGrowth` | Flat knob (0.08, `def` from `REPAIR_MILESTONE_GROWTH`), CS042 P6, **POWERUPS** section | **It is the thing that stops a fixed interval acting as an inverse lever.** `nextRepair += REPAIR_MILESTONE × (1 + growth × wave)` widens the gap between score milestones as the level climbs, because score *rate* climbs on its own and a flat interval therefore paid out faster and faster the deeper a run went. That makes this a *counter*-scaling correction to an existing wave-dependence rather than a new one — which is exactly why it is a knob on the interval and not a `LEVERS` row of its own. At 0 the interval is flat and CS040's arithmetic returns byte-for-byte. `REPAIR_MILESTONE` itself is unmoved at 10,000 (CS040's FORK-B). |
| `repairMilestoneHullPct` | Flat knob (0.70, `def` from `REPAIR_MILESTONE_HULL_PCT`), CS042 P6, **POWERUPS** section | **A player-state threshold, like the `healthGap*` pity pairs — it reads hull, never the level clock.** A milestone crossing pays only while `hp <= SHIP_MAX_HP × this`, so a scratch no longer buys a pickup. ⛔ **It is the SAME NUMBER as `bankSpareHullPct` by design** — CS042 has one definition of "hurt" — and the suite asserts the two are *equal* rather than each being 0.70, so a one-sided retune fails. At 1.0 it reduces to CS040 P1's `hp < SHIP_MAX_HP` rule exactly, because that older clause is deliberately kept beside it. |
| `cargoUnitMass` | Flat knob (0.07, `def` from `CARGO_UNIT_MASS`), CS042 P7, **SHIP** section | **A physical property of the cargo, not a difficulty axis — and it is the ONE knob the whole handling model now hangs on.** `shipMass()` = `1 + chainMass() × this`, and acceleration, the drag rate, the turn rate and the momentum tug all divide by that. The difficulty it produces is already levered elsewhere, by how much the player *chooses* to tow and by `payloadSlots(n)`'s own fixed curve; levering the mass too would escalate the same signal twice. ⛔ **At 0 the ship is weightless however much it tows** — thrust, drag, turn and tug all read unloaded at once — which is the clean one-knob A/B for §6.8 as a whole. Its partner is `engineMassMult`. `CHAIN_TUG` (58) is **derived** from this default and must be re-solved if it moves. |
| `bankSpareHullPct` | Flat knob (0.70, `def` from `BANK_SPARE_HULL_PCT`), CS042 P9, **POWERUPS** section | **A choice threshold on a player resource, the `healthBankMax` shape rather than a pressure axis.** It decides which of two jobs a banked Health charge does on a damage event: above it the charge spares a scoop level, at or below it the charge heals. Neither end is an off switch — at **1.0** the charge always heals and the scoop always drops (CS040's rule at CS042's faster loss rate), at **0.0** a charge always spares while a scoop exists. It reads the **post-damage** hull, which is why it is a threshold and not a full-hull test. Moves with `repairMilestoneHullPct` or not at all. |
| `menuRepeatDelay` / `menuRepeatRate` | Flat knobs (0.40 s / 0.10 s, `def` from `MENU_REPEAT_DELAY`/`MENU_REPEAT_RATE`), CS042 P10, **GLOBAL** section | **Input feel on a menu, which is not gameplay at all** — the same category as `earlyWorldLevels`' boundary or the level banner's four look-call knobs, and about as far from `game.wave` as a knob in this registry gets. They pace the game's own held-key repeat for menu navigation (up/down only); nothing they touch is reachable while the sim is running at difficulty. |
...
- **CS042 GATE C (between P10 and P11). ⛔ NOT a clean gate on the build, but CLEAN ON
  THIS DOCUMENT'S SUBJECT: no lever moved and no knob moved.** G1/G2/G3/G5/G6/G7/G8/G9/G10
  all came back "ship as-is", and both open flags closed at their shipped values —
  **FLAG-CS042-l** (`cargoUnitMass` 0.07) and **FLAG-CS042-m** (the turn penalty it
  introduces, 241 → 90 °/s empty-to-full) are "fine as shipped". The seven knobs CS042
  added (`healthSpawnLock`, `repairMilestoneGrowth`, `repairMilestoneHullPct`,
  `cargoUnitMass`, `bankSpareHullPct`, `menuRepeatDelay`, `menuRepeatRate`) all stand at
  their analytic defaults, unplaytested against their own extremes — each row in §4 names
  what its A/B is. **G4 was the one actionable finding and it is render-only** (the scoop
  mouth and the level-6/7 orbs read as hittable hull; P11 answered it with a dashed field
  stroke), so it moved no number this document owns. ⚠ **Two knobs did move earlier in the
  changeset, by phase rather than by gate**, and both are recorded as reversals rather than
  retunes: `scoopHitsPerLevel` 5 → **2** (P9, reversing v3.4 P3's own stated intent) and
  the four `healthGap*` pairs widened to 30 / 45 / 10 / 16 (P6). ⚠ **`SHIP_DRAG` stays at
  0.35** — 0.45 was flown at GATE A and rejected (FLAG-CS042-f, "no change").
