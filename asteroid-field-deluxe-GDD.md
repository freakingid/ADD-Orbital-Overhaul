...
**Version:** 1.5 shipped · v2.0 in progress (Phase 4 of 9 done — see companion docs below)
...
6. **Ease players in.** (v1.5) Opening waves should be approachable; intensity ramps gradually with wave number, not all at once. Any threat parameter (speed, fire rate, accuracy, spawn frequency, density) is expressed as a value that interpolates from an easy wave-1 **floor** up to a full-intensity **ceiling** through one shared, wave-driven `difficultyFactor(wave)` curve (§2.13), so the whole game's pacing tunes from one knob (`RAMP_WAVES`). Complements Pillar 3 — pressure should arrive *on a schedule*, not slam the player at wave 1. As of v1.5 the saucers scale off this curve; the Hunter Satellite redesign (planned) wires into the same curve from the start.
...
- **What will scale next (not built this phase):** the **Hunter Satellite** redesign (F4/Phase 5) is specified to wire its drift/homing speeds and turn rates into this same `ramp`/`difficultyFactor` from the start — the F4 values become the ceilings the curve climbs toward. The **Debris Satellite density** retune (joint F3+F10 pass) also lands later; it leans on `GARBAGE_DECAY` for density control (§2.10.1) rather than on the difficulty curve. Neither is wired yet — v1.5 is the difficulty *helper* plus the *saucer* application only.
...
- **v2.0 (in progress)** — Phases 1–4 shipped (larger scrolling world as v1.2; HP & knockback as v1.3; Debris Satellites + variable-mass garbage as v1.4; difficulty ramp + saucer calming as v1.5). Still planned: a Hunter Satellite redesign of the killer satellite/wedge system (wiring into the v1.5 difficulty ramp), weapon/health/utility powerups, controller support, pause/options menu with rebindable controls, and an achievements system; plus the still-open joint F3+F10 Debris-density retune. Full specs in `PLANNED-FEATURES-v2.md`; build order in `IMPLEMENTATION-PHASES.md`.
